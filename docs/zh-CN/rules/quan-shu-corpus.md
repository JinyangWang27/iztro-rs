...
| `fortune.tang_ju_kong_wang.gain_loss_critical` | 倘居空亡，得失最为要紧 | `executable` | 保守取命宫本身见已建模空亡族星；source-hit-only，无 ClaimSpec。 |
...
| `risk.sha_luo_kong_wang.malefic_force_voided` | 杀落空亡竟无威力 | `executable` | 保守取七杀与已建模空亡族星同宫；source-hit-only，无 ClaimSpec。 |
| `life.shen_zuo_kong_wang.body_void_pivot` | 身坐空亡论荣枯，专求其要 | `executable` | 保守取身宫见已建模空亡族星；source-hit-only，无 ClaimSpec。 |
| `fortune.fu_de_yu_kong_jie.restless_spirit` | 福德遇空劫，奔走无力 | `executable` | 保守取福德宫见地空或地劫（`StarTag::KongJie`）；source-hit-only，无 ClaimSpec。 |
...
**unlinked source items 为 0**。当前已结构化 source inventory 链接后的规则按 `status`
分布如下（见覆盖报告）：

| status | 数量 | 说明 |
| --- | ---: | --- |
| `executable` | 9 | 马遇空亡、日月反背、贪居亥子、刑遇贪狼及本批空亡/空劫 source-hit-only 规则（已接线谓词）。 |
| `normalized` | 40 | 句意结构清晰，但所依赖事实（如十二长生、夹宫目标宫、星曜化气、流年杂曜等）尚未建模，暂不可执行。 |
