...
`rule-corpus/patterns/rules.toml`：其中既可以有项目自有规则（`work =
"iztro_pattern_catalog"` 与 `source_id = "pattern.*"`），也可以有全书出处格局规则
（`work = "zi_wei_dou_shu_quan_shu"` 与 `source_id = "quan_shu.v01.*"`）。两个语料在
编译期一并嵌入，运行期由 `classical_rules()` 合并供 `evaluate_classical` 使用；
`quan_shu_rules()` / `pattern_rules()` 分别只返回各自语料文件中的规则。
...
| --- | --- | --- |
| `定富局` | `pattern_rule` | source inventory 已分段；`金灿光辉` 通过 `core::pattern` 检测，并在 pattern runtime corpus 中接入 classical runtime。 |
| `定贵局` | `pattern_rule` | source inventory 已分段；`日出扶桑`、`月落亥宫`、`月生沧海`、`马头带剑`、`贪火相逢`、`武曲守垣` 等保守子集可执行并接入 runtime。 |
| `定贫贱局` | `pattern_rule` | source inventory 已分段；`财与囚仇`、`马落空亡` 等保守子集可执行并接入 runtime。 |
| `定杂局` | `pattern_rule` | source inventory 已分段；限运条件尚未在本 PR 中建模，暂不执行。 |

这些格局目录的完整清单仍由 source inventory 管理；运行时代码不解析 inventory。只有保守、
结构清晰且已有检测器的子集，才在 `rule-corpus/patterns/rules.toml` 中登记为
pattern-derived runtime rules，并由 `rules/classical/evaluator.rs` 复用 `core::pattern`
检测结果产出 `ClassicalSourceHit`。其中语义清晰且不重复者可带 `[rule.claim]`；`马落空亡`
因与既有太微赋 `马遇空亡` 判断重叠，刻意只产出 source hit，不新增重复 Claim。
未实现、只写「见前批注」或依赖限运的条目仅保留为 source inventory。
...
`pattern_rule` source group 可保持 segmented/unlinked；当某个条目具备保守可执行条件时，
由 `core::pattern` 提供结构检测，并在 `rule-corpus/patterns/rules.toml` 中用全书
`source_id` 显式接入 classical runtime。
