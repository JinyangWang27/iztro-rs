...
身份：其 `PatternId`，由 `core::pattern` 检测；《全书》格局目录条目是这些规范格局的古籍
出处来源，通过 `core::pattern::metadata` 挂到 `PatternId` 上，不在本文件中创建独立的
...
| --- | --- | --- |
| `定富局` | `pattern_rule` | source inventory 已分段；`金灿光辉` 由 `core::pattern` 检测为规范 `PatternId`，出处经 `pattern_source_metadata` 挂接。 |
| `定贵局` | `pattern_rule` | source inventory 已分段；`日出扶桑`、`月落亥宫`、`月生沧海`、`马头带剑`、`贪火相逢`、`武曲守垣` 等保守子集由 `core::pattern` 检测。 |
| `定贫贱局` | `pattern_rule` | source inventory 已分段；`财与囚仇`、`马落空亡` 等保守子集由 `core::pattern` 检测。 |
| `定杂局` | `pattern_rule` | source inventory 已分段；限运条件尚未在本 PR 中建模，暂不检测。 |

这些格局目录的完整清单仍由 source inventory 管理；运行时代码不解析 inventory。只有保守、
结构清晰的子集才在 `core::pattern` 中实现为规范 `PatternId` 检测器，产出 `PatternDetection`；
其《全书》出处通过 `core::pattern::metadata::pattern_source_metadata` 挂接，供 GUI／文档展示。
...
条件时，由 `core::pattern` 提供结构检测（规范 `PatternId`），其《全书》出处通过
