...
| --- | --- | --- |
| `定富局` | `pattern_rule` | source inventory 已分段；仅结构清晰者通过 `core::pattern` 产出 `PatternDetection`。 |
| `定贵局` | `pattern_rule` | source inventory 已分段；`日出扶桑`、`月落亥宫`、`月生沧海`、`马头带剑`、`贪火相逢`、`武曲守垣` 等保守子集可执行。 |
| `定贫贱局` | `pattern_rule` | source inventory 已分段；`财与囚仇`、`马落空亡` 等保守子集可执行。 |
| `定杂局` | `pattern_rule` | source inventory 已分段；限运条件尚未在本 PR 中建模，暂不执行。 |

这些格局目录不是 ordinary classical claim rule：不新增 `ClaimSpec`，不新增 i18n 判断文本，
不在 `rules/classical/evaluator.rs` 接线，也不让 `evaluate_classical` 产出重复判断。已实现
格局只通过 `PatternDetection` 暴露结构化事实；未实现条目仅保留为 source inventory。
...
`pattern_rule` source group 可保持 segmented/unlinked，用于后续接入 `core::pattern` 的静态
source metadata，而不是 classical claim rule。
