...
`rule-corpus/quan-shu/` 只收录普通全书断语规则（`work =
"zi_wei_dou_shu_quan_shu"`）。格局衍生的运行时规则收录于
`rule-corpus/patterns/rules.toml`，且**只放项目自有规则**（`work =
"iztro_pattern_catalog"` 与 `source_id = "pattern.*"`）。一个格局只有唯一的规范运行时
身份：其 `PatternId`，由 `rules::pattern` 检测；《全书》格局目录条目是这些规范格局的古籍
出处来源，通过 `rules::pattern::metadata` 挂到 `PatternId` 上，不在本文件中创建独立的
classical 运行时规则。两个语料在编译期一并嵌入，运行期由 `classical_rules()` 合并供
`evaluate_classical` 使用；`quan_shu_rules()` / `pattern_rules()` 分别只返回各自语料文件中
的规则。
...
| `career.tai_yang_ju_wu.authority_status` | 太阳居午，谓之日丽中天，有专权之贵，敌国之富 | `executable` | 保守取太阳居午宫即触发（不要求守命或单守，区别于格局金灿光辉）。 |
| `career.tai_yin_ju_zi.clean_office_admonition` | 太阴居子，号曰水澄桂萼，得清要之职，忠谏之材 | `executable` | 保守取太阴居子宫即触发（不要求守命）。 |

卷一出处格局目录：

| section | category | 运行时关系 |
| --- | --- | --- |
| `定富局` | `pattern_rule` | source inventory 已分段；`金灿光辉` 由 `rules::pattern` 检测为规范 `PatternId`，出处经 `pattern_source_metadata` 挂接。 |
| `定贵局` | `pattern_rule` | source inventory 已分段；`日出扶桑`、`月落亥宫`、`月生沧海`、`马头带剑`、`贪火相逢`、`武曲守垣` 等保守子集由 `rules::pattern` 检测。 |
| `定贫贱局` | `pattern_rule` | source inventory 已分段；`财与囚仇`、`马落空亡` 等保守子集由 `rules::pattern` 检测。 |
| `定杂局` | `pattern_rule` | source inventory 已分段；限运条件尚未在本 PR 中建模，暂不检测。 |

这些格局目录的完整清单仍由 source inventory 管理；运行时代码不解析 inventory。只有保守、
结构清晰的子集才在 `rules::pattern` 中实现为规范 `PatternId` 检测器，产出 `PatternDetection`；
其《全书》出处通过 `rules::pattern::metadata::pattern_source_metadata` 挂接，供 GUI／文档展示。
`rules::classical` **不**为这些格局目录条目创建重复的 source-hit/claim 规则，`evaluate_classical`
也不消费 pattern 检测。现代教材可为条件设计提供参考，但不创造独立的格局身份。未实现、
只写「见前批注」或依赖限运的条目仅保留为 source inventory。

项目自有 pattern 规则（`rule-corpus/patterns/rules.toml` 中
`work = "iztro_pattern_catalog"` 的条目）：
...
`pattern_rule` source group 在 inventory 中保持 segmented/unlinked；当某个条目具备保守可执行
条件时，由 `rules::pattern` 提供结构检测（规范 `PatternId`），其《全书》出处通过
`pattern_source_metadata` 挂接，不在 `rule-corpus/patterns/rules.toml` 中创建 classical 运行时规则。

## 太微赋规范化补全

卷一「太微赋」的全部 rule-candidate 出处单元现已链接到运行时规则元数据，覆盖报告中
太微赋 **unlinked source items 为 0**。「斗数骨髓赋」的规则链接分批推进：第一批为夹宫
六句（夹贵夹禄／夹权夹科／夹日夹月／夹昌夹曲／夹空夹劫／夹羊夹陀，均为 `normalized`），
其余骨髓赋 source item 保持 segmented/unlinked。全部已链接规则按 `status` 分布如下
（见覆盖报告）：

| status | 数量 | 说明 |
| --- | ---: | --- |
| `executable` | 7 | 马遇空亡、日月反背、贪居亥子、刑遇贪狼、善福居空位、太阳居午、太阴居子（已接线谓词）。 |
| `normalized` | 45 | 句意结构清晰，但所依赖事实（如十二长生、夹宫目标宫、星曜化气、流年杂曜等）尚未建模，暂不可执行。含斗数骨髓赋夹宫六句第一批链接。 |
