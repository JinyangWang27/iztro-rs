...
注意一处与分层架构长期形态的刻意偏离：规则谓词目前直接读取 `Chart` 事实，
而非消费特征提取层的 `ChartFeatures`。谓词需要 `ChartFeatures` 尚未承载的
盘面细节（夹宫几何、亮度、空亡策略）。特征 → 规则的串联留待特征结构稳定后
再做；在此之前两层刻意并行。
...
### 将 normalized 规则晋升为 executable

可执行覆盖逐条增长，绝不批量（每个 PR 1–2 条）：

1. 选一条条件可用已建模盘面事实表达的 `status = "normalized"` 规则；
2. 撰写其 `[rule.claim]` 块（领域、主题、吉凶、基础强度、`claim_key`），
   并在 i18n `claims.ftl` 资源中补充对应判断文案；
3. 在 `evaluator.rs` 实现谓词分支，复用 `rules::query` 助手——若所需盘面事实
   尚未建模，则返回类型化的 `RuleOutcome::Unsupported`（如 `LU_MA_JIAO_CHI`）；
4. 补充正反例规则测试（`tests/classical_rules.rs`）；
5. 将 `status` 翻转为 `executable`（fixture 覆盖充分后可标 `tested`）。

评估器的测试守卫强制每条 `executable`/`tested` 语料规则都有谓词分支，
遗漏任一步都会显式失败。
...
### 运行时卷节表（`sections.toml`）

运行时唯一可用的卷节元数据是小型表
`rule-corpus/quan-shu/sections.toml`：inventory 中每个 `source_group` 对应一条，
仅含 `source_id_prefix`、`work`、`volume`、`section` 四个字段。它经
`include_str!` 内嵌，并以
`rules::source::source_section(source_id) -> Option<&SourceSection>` 暴露：
将 id 截断到最后一个 `.` 之后做精确前缀查找（inventory 的 item key 不含点号）。
显示层用它为经典规则命中与有出处的格局检测标注卷/节
（`PatternSourceMetadata` 有意不再自存 `section`）。

该表有意不存 `anchor`（恒等于 `section`）与 `doc_path`（恒可由卷号推导为
`docs/zh-CN/sources/quan_shu/volume-{volume:02}.md`）。
`crates/iztro/tests/source_sections.rs` 双向校验该表与 inventory 的同步，
并强制上述两条可推导性不变式。item 级数据与治理字段（`status`、`category`、
`linked_rule_ids`、`source_order`）仍为 test-only；见 ADR 0010。
