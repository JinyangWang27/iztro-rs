...
- [ ] 添加其余杂曜。iztro 2.5.8 默认算法的 38 颗杂曜中仍剩 12 颗；逐星落点基准与状态见兼容性文档「其余本命杂曜/辅助星清单」。下一个子集：出生年支组（咸池/天空）。
- [ ] 添加阳历转农历、闰月行为、早晚子时变体、时间范围星曜和 bindings。

当前核心切片：`by_lunar` 接受显式农历输入以及显式出生年干、年支，生成确定性的本命星盘事实，并用选定的 `iztro` 2.5.8 fixtures 校验 minimal chart 字段、十四主星、十四颗已支持辅星，以及二十六颗已支持本命杂曜/辅助星。其余未支持杂曜、阳历转农历、闰月行为、早晚子时变体、时间范围星曜和 bindings 仍然推迟。
