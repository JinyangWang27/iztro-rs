...
1. `crates/iztro/rule-corpus/quan-shu/source/` 下的结构化 source inventory。
2. 每条 source item 的稳定 `source_id`、`volume`、`anchor`、`status` 和 `source_text_zh_hans`。
3. 现有 pilot rules 到 source items 的链接。
4. corpus lint，确保 rule 引用的 `source_id` 必须存在。
5. coverage report，追踪每条原文处于 raw / segmented / normalized / executable / tested / ambiguous / rejected 中的哪一类。
