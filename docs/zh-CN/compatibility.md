...
类型。只有当请求的月份确实是该年的闰月时，闰月标志才被采纳，复现上游 `lunar2solar`。
...
完整 facade 序列化对齐、bindings、特征提取、规则与叙事仍延期实现。
...
最小 `by_solar`（ICU4X 支持的阳历转农历）、已支持 `by_lunar`/`by_solar` 切片的
...
规则与叙事。`by_solar` 的转换由内部 ICU4X（`icu_calendar`）支持，不出现在公开 API 中。
重新生成：
