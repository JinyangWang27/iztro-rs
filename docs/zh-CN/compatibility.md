...
- 阳历转农历，以及 `by_solar` 的可配置年分界出生年/四柱事实推导，置于内部 `core/calendar` 适配器之后，该适配器由 `lunar-lite` 支撑；
...
`by_solar` 校验阳历日期，通过内部 `core/calendar` 适配器（由 `lunar-lite` 支撑）取得历法事实，并通过 `lunar-lite` 的四柱 API 推导事实性本命四柱：无歧义的日柱与时柱（连续日数、五鼠遁时柱，含晚子时换日）、月柱（五虎遁）与年柱均按所配置的分界推导。`iztro-rs` 只负责策略选择：`YearBoundary` 映射到 `lunar-lite` 的 `YearDivide`（农历新年 / 立春分界），月柱使用常规 `MonthDivide`（五虎遁）。它设置 `is_leap_month` 与 `fix_leap`，委托 `by_lunar` 安星，并把结果（`lunar-lite` 的 `FourPillars` 值对象）保留在 `Chart`。它本身不添加安星逻辑。显式 invariant：当 `Chart::four_pillars()` 存在时，其年柱等于 `Chart::birth_year()`。

真太阳时仍是 `iztro-rs` 计算策略（`SolarTimePolicy::ApparentSolarTime`），在 `lunar-lite` 历法转换**之前**对民用钟表时间应用；历法只接收已解析的本地日期与时辰索引。

`ChartCalculationConfig` 是 input/runtime calculation-policy 维度，不是算法或盘面维度。它包含 `SolarTimePolicy`、`YearBoundary`、`LeapMonthBoundary` 与 `NominalAgeBoundary`。`YearBoundary` 将上游 `yearDivide: "normal"` 映射到 `ChineseNewYearEve`，将 `"exact"` 映射到 `LiChun`；`ChineseNewYearEve` 表示上一年持续到除夕结束，新干支年从正月初一开始。`YearBoundary::LiChun` 映射到 `lunar-lite` 的 `YearDivide::Exact`，按**日期**粒度判定立春分界：立春当日属于新干支年，故同一立春当日、不同钟表时间出生者共享同一年柱，与 `iztro@2.5.8` 一致。兼容优先级为：(1) `iztro@2.5.8` supported-field fixture 一致性；(2) 来自 `lunar-lite` 的正确历法语义。采用日期级分界后，`year_divide_exact_2000_02_04` 重新与上游一致（`庚辰`）；不再有有意偏离，所有 fixture 用例保持与上游严格一致。时刻级（精确瞬时）立春分界需要 `lunar-lite` 暴露节气瞬时基元，暂作为后续单独改动。`LeapMonthBoundary` 将上游 `fixLeap: false` 映射到 `AsPreviousMonth`，将 `fixLeap: true` 映射到 `MidMonth`。`NominalAgeBoundary` 将上游 `ageDivide: "normal"` 映射到 `NaturalYear`，将 `"birthday"` 映射到 `Birthday`，且只用于 runtime/full-horoscope 虚岁解析。
...
`lunar-lite` 拥有底层天干、地支、干支与四柱值对象（`HeavenlyStem`、`EarthlyBranch`、`StemBranch`、`FourPillars`），`iztro-rs` 直接使用并从 `core` re-export；`lunar-lite` 是置于 `core/calendar` 之后的历法/干支依赖，其历法转换保留在该适配器内部。`core` 拥有紫微斗数特有的纳音与五行局逻辑。

`lunar-lite` 历法转换限定在 `core/calendar` 模块内。集成测试不直接依赖历法后端行为；应使用已提交 fixture facts，或通过 `iztro-rs` API 穿过内部历法适配器边界。
