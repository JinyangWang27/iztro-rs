...
生成的命盘从 `StaticChartViewSnapshot` 渲染，使用内置的思源宋体（Source Han Serif SC）
...
长生/博士在左下，将前/岁前在右下。一个紧凑图例解释色调含义。这纯粹是对核心 view model
...
请求一份新预备的 `StaticChartViewSnapshot`，选择某个父级会清除更深的选择。
...
求值或格局检测（检视面板的缓存模型见 [`current-status.md`](current-status.md)）。除了呈现
