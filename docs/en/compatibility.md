...
Apparent solar time remains an `iztro-rs` calculation policy (`SolarTimePolicy::ApparentSolarTime`), applied to the civil clock time *before* the `lunar-lite` calendar conversion runs; the calendar only ever receives the already-resolved local date and 时辰 index.
