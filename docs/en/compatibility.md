...
- serializable `HoroscopeFacadeSnapshot` export that combines the modeled full horoscope surface — the `HoroscopeSupportedFieldsSnapshot` blocks, the `HoroscopeRuntime` Life-palace projections, and the retained numeric target context (solar date, lunar date with leap-month flag, and target time index) — into one upstream-like payload, fixture-backed against `crates/iztro/fixtures/iztro/horoscope_facade.json`;
