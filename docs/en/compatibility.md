...
- solar-to-lunar conversion and boundary-configurable birth-year/four-pillar derivation for `by_solar`, behind the internal `core/calendar` adapter backed by `lunar-lite`;
...
- renderer-neutral `ChartStackSnapshot`, GUI-facing `StaticChartProjection`, and a deterministic plain text renderer demo.
...
`by_solar` validates the Gregorian/solar date, derives calendar facts through the internal `core/calendar` adapter backed by `lunar-lite`, and derives factual natal four pillars. The unambiguous day and hour pillars (continuous day count and 五鼠遁 hour, including the 晚子时 day roll) come from `lunar-lite`'s four-pillar API directly. The year and month pillars are recomputed by the `core/calendar` adapter from the configured `YearBoundary`: the year pillar follows the lunar-new-year or datetime-level 立春 boundary, and the month pillar uses the normal 五虎遁 derived from the effective year stem. `iztro-rs` owns this policy selection. It sets `is_leap_month` and `fix_leap`, delegates placement to `by_lunar`, and retains the resulting `FourPillars` (a `lunar-lite` value object) on `Chart`. It adds no placement logic of its own. The invariant is explicit: when `Chart::four_pillars()` is present, its year pillar equals `Chart::birth_year()`.

Apparent solar time remains an `iztro-rs` calculation policy (`SolarTimePolicy::ApparentSolarTime`), applied to the civil clock time *before* the `lunar-lite` calendar conversion runs; the adapter receives the already-resolved local date, resolved `timeIndex`, and resolved clock hour/minute for the LiChun comparison.

`ChartCalculationConfig` is an input/runtime calculation-policy axis, not an algorithm or chart-plane axis. It contains `SolarTimePolicy`, `YearBoundary`, `LeapMonthBoundary`, and `NominalAgeBoundary`. `YearBoundary` maps upstream `yearDivide: "normal"` to `ChineseNewYearEve` and `"exact"` to `LiChun`; `ChineseNewYearEve` means the previous year lasts through 除夕 and the new cyclic year begins at 正月初一. `YearBoundary::LiChun` is resolved at **datetime** granularity using `lunar_lite::li_chun_datetime`: a birth before the exact 立春 instant on the 立春 day keeps the previous Ganzhi year, while a birth at or after it advances, so two births on the 立春 day can fall in different year pillars. This intentionally diverges from upstream `iztro@2.5.8`, which is date-level. `LeapMonthBoundary` maps upstream `fixLeap: false` to `AsPreviousMonth` and `fixLeap: true` to `MidMonth`. `NominalAgeBoundary` maps upstream `ageDivide: "normal"` to `NaturalYear` and `"birthday"` to `Birthday`, and is applied only when resolving runtime/full-horoscope nominal age.

`YearBoundary::LiChun` is datetime-level, powered by `lunar_lite::li_chun_datetime`. The compatibility priority order is: (1) `iztro@2.5.8` supported-field fixture parity; (2) explicit `iztro-rs` semantic corrections where documented; (3) lower-level `lunar-lite` compatibility. The datetime-level 立春 case is an intentional documented divergence under priority (2): `year_divide_exact_2000_02_04` is a birth at 08:00 on 2000-02-04, before the 20:40:24 立春 instant, so `iztro-rs` keeps the previous Ganzhi year `己卯` rather than upstream's date-level `庚辰`. Every other fixture case keeps strict upstream parity.
...
`lunar-lite` owns the low-level stem, branch, stem-branch, and four-pillar value objects (`HeavenlyStem`, `EarthlyBranch`, `StemBranch`, `FourPillars`), which `iztro-rs` uses directly and re-exports from `core`; `lunar-lite` is the calendar/GanZhi dependency behind `core/calendar` and its calendar conversion stays internal to that adapter. `core` owns Zi Wei-specific NaYin and five-element bureau logic.

`lunar-lite` calendar conversion is confined to the `core/calendar` modules.
Integration tests do not depend on calendar-backend behavior directly; they use
committed fixture facts or `iztro-rs` APIs that cross the internal calendar
adapter boundary.
