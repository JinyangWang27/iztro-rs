...
`by_solar` validates the Gregorian/solar date, derives calendar facts through the internal `core/calendar` adapter backed by `lunar-lite`, and derives factual natal four pillars through `lunar-lite`'s four-pillar API: the unambiguous day and hour pillars (continuous day count and 五鼠遁 hour, including the 晚子时 day roll), the month pillar (五虎遁), and the year pillar all follow the configured boundaries. `iztro-rs` owns only the policy selection: `YearBoundary` maps to `lunar-lite`'s `YearDivide` (lunar-new-year / 立春 boundary), and the month uses the normal `MonthDivide` (五虎遁). It sets `is_leap_month` and `fix_leap`, delegates placement to `by_lunar`, and retains the resulting `FourPillars` (a `lunar-lite` value object) on `Chart`. It adds no placement logic of its own. The invariant is explicit: when `Chart::four_pillars()` is present, its year pillar equals `Chart::birth_year()`.
...
`ChartCalculationConfig` is an input/runtime calculation-policy axis, not an algorithm or chart-plane axis. It contains `SolarTimePolicy`, `YearBoundary`, `LeapMonthBoundary`, and `NominalAgeBoundary`. `YearBoundary` maps upstream `yearDivide: "normal"` to `ChineseNewYearEve` and `"exact"` to `LiChun`; `ChineseNewYearEve` means the previous year lasts through 除夕 and the new cyclic year begins at 正月初一. `YearBoundary::LiChun` maps to `lunar-lite`'s `YearDivide::Exact`, which resolves the 立春 boundary at **date** granularity: the 立春 day belongs to the new Ganzhi year, so two births on the 立春 day share the same year pillar regardless of clock time. This matches upstream `iztro@2.5.8`. `LeapMonthBoundary` maps upstream `fixLeap: false` to `AsPreviousMonth` and `fixLeap: true` to `MidMonth`. `NominalAgeBoundary` maps upstream `ageDivide: "normal"` to `NaturalYear` and `"birthday"` to `Birthday`, and is applied only when resolving runtime/full-horoscope nominal age.

`YearBoundary::LiChun` follows `lunar-lite`'s date-level 立春 boundary, matching upstream `iztro@2.5.8`. The compatibility priority order is: (1) `iztro@2.5.8` supported-field fixture parity; (2) correct calendar semantics from `lunar-lite`. With the date-level boundary, `year_divide_exact_2000_02_04` again matches upstream (`庚辰`); there is no intentional divergence, and every fixture case keeps strict upstream parity. A datetime-level (exact-instant) 立春 boundary would require `lunar-lite` to expose a solar-term-instant primitive and is deferred to a separate change.
