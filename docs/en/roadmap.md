...
- [x] Expose factual natal four pillars in the facade snapshots. `NatalFacadeSnapshot` carries an optional `NatalFacadeFourPillarsSnapshot` (`four_pillars`) reusing `lunar_lite::FourPillars`: each pillar stays a machine-readable `StemBranch` with an additive zh-CN `*_zh` label. It is `Some(..)` for `by_solar`-derived charts (its year pillar equals `Chart::birth_year()`), omitted/`None` for `by_lunar`-derived charts, and carried through the embedded `astrolabe` of `HoroscopeFacadeSnapshot`. Factual export only — no 十神, 藏干, 五行 scoring, 喜用神, 成格, readings, or other BaZi interpretation.
- [ ] Add full facade serialization parity. Add complete upstream astrolabe helper/query methods, localized natal labels, localized `lunarDate`/`solarDate` strings, full BaZi interpretation/strings, decadal ranges, age arrays, and the runtime query helpers — all currently deferred from `HoroscopeFacadeSnapshot`.
