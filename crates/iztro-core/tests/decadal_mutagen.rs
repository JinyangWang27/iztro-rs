use iztro_core::{
    BirthContext, CalendarDate, Chart, DecadalMutagenLayerInput, EarthlyBranch, Gender,
    HeavenlyStem, LunarDay, LunarMonth, MethodProfile, Mutagen, NatalChartWithMajorStarsInput,
    NatalChartWithSupportedStarsInput, Scope, StarName, StemBranch, TemporalContext, TemporalLayer,
    YearlyMutagenLayerInput, birth_year_star_mutagen, build_decadal_mutagen_layer,
    build_natal_chart_with_major_stars, build_natal_chart_with_supported_stars,
    build_yearly_mutagen_layer,
};
...
/// Explicit decadal starting age used across the cases; the builder records it
/// verbatim and derives no age range from it.
const START_AGE: u8 = 24;
...
fn supported_star_natal_chart() -> Chart {
    build_natal_chart_with_supported_stars(NatalChartWithSupportedStarsInput::new(
        ...
        MethodProfile::placeholder("decadal_mutagen_test_profile"),
...
#[test]
fn decadal_layer_has_decadal_scope_context_and_activation_shape() {
    let natal = supported_star_natal_chart();
    let layer = build_decadal_mutagen_layer(
        &natal,
...
#[test]
fn building_a_decadal_layer_leaves_the_natal_chart_unchanged() {
    let natal = supported_star_natal_chart();
    ...
    let _layer = build_decadal_mutagen_layer(
        &natal,
        DecadalMutagenLayerInput::new(geng_stem_branch(), START_AGE),
    )
    .expect("decadal mutagen layer should build");
...
fn absent_target_stars_are_skipped_rather_than_invented() {
    ...
    let major_layer = build_decadal_mutagen_layer(
        &major_only,
        DecadalMutagenLayerInput::new(xin_stem_branch(), START_AGE),
    )
    .expect("decadal mutagen layer should build for a major-only chart");
    let full_layer = build_decadal_mutagen_layer(
        &supported,
        DecadalMutagenLayerInput::new(xin_stem_branch(), START_AGE),
    )
    .expect("decadal mutagen layer should build for a full chart");
...
#[test]
fn decadal_layer_holds_no_placements_and_only_references_natal_stars() {
    let natal = supported_star_natal_chart();
    let layer = build_decadal_mutagen_layer(
        &natal,
        DecadalMutagenLayerInput::new(geng_stem_branch(), START_AGE),
...
#[test]
fn decadal_layer_round_trips_through_json() {
    let natal = supported_star_natal_chart();
    let layer = build_decadal_mutagen_layer(
        &natal,
        DecadalMutagenLayerInput::new(geng_stem_branch(), START_AGE),
    ...
    let encoded = serde_json::to_string(&layer).expect("decadal layer should serialize");
    let decoded: TemporalLayer =
        serde_json::from_str(&encoded).expect("decadal layer should deserialize");

    assert_eq!(decoded, layer);
    assert_eq!(decoded.scope(), Scope::Decadal);
    assert!(decoded.placements().is_empty());
}

#[test]
fn decadal_and_yearly_builders_agree_on_activation_triples() {
    let natal = supported_star_natal_chart();

    let decadal = build_decadal_mutagen_layer(
        &natal,
        DecadalMutagenLayerInput::new(geng_stem_branch(), START_AGE),
    )
    .expect("decadal mutagen layer should build");
    ...
    // source_scope and TemporalContext. Both walk the same natal chart through
    // the same shared helper, so the triples appear in identical order.
    let decadal_triples: Vec<(StarName, EarthlyBranch, Mutagen)> = decadal
        .activations()
        .iter()
        .map(|a| (a.target_star(), a.target_branch(), a.mutagen()))
        .collect();
    let yearly_triples: Vec<(StarName, EarthlyBranch, Mutagen)> = yearly
        .activations()
        .iter()
        .map(|a| (a.target_star(), a.target_branch(), a.mutagen()))
        .collect();
    assert_eq!(decadal_triples, yearly_triples);

    // The layers still differ in scope and typed context.
    assert_ne!(decadal.scope(), yearly.scope());
    assert_ne!(decadal.context(), yearly.context());
}
