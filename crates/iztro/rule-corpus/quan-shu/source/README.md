...
source Markdown
  ...
  -> QuanShu rule metadata TOML   -> Rust predicates / evaluator -> Claim[]
     (aphorism rules)
  -> pattern source provenance    -> rules::pattern detector      -> PatternDetection[]
     (rules::pattern::metadata, attached to a canonical PatternId)
```

A 格局/pattern has a single canonical runtime identity (`PatternId`). QuanShu
pattern catalogue entries are source provenance attached to that `PatternId` via
`rules::pattern::metadata`; they do **not** create a parallel pattern-derived
classical runtime rule, and `evaluate_classical` does not consume pattern
detections.
...
- This source inventory TOML is machine-checkable corpus tracking only. It is **not** part of the runtime chart-evaluation path: nothing in `crates/iztro/src/` parses it, `evaluate_classical` does not depend on it, and the Markdown volumes are never parsed at runtime. The one exception is section metadata: the small runtime table `rule-corpus/quan-shu/sections.toml` (one entry per `source_group`, embedded by `rules::source::source_section`) lets display layers cite 卷/节. It is kept in sync with this inventory by `crates/iztro/tests/source_sections.rs`; all item-level data and governance fields (`status`, `category`, `linked_rule_ids`, `source_order`) remain test-only.
- Source-backed pattern detections carry small static provenance in `rules::pattern::metadata` only after a pattern has an executable detector. This is provenance attached to a canonical `PatternId`; it is **not** a classical runtime rule. `evaluate_classical` never emits a source hit or claim for a QuanShu pattern catalogue entry — those are detected only as `PatternDetection`s by `rules::pattern`.
- `crates/iztro/tests/classical_source_inventory.rs` validates the inventory and its QuanShu aphorism-rule links to `crates/iztro/rule-corpus/quan-shu/rules.toml` using private, test-only structs. It asserts that the inventory parses, has unique `source_id`s, has non-empty required fields, that 太微赋 and the four source-backed pattern sections have continuous section-local `source_order`, that source ids are stable mnemonics (not purely numeric), that every `rule_linked` item is linked, that every QuanShu rule `source_id` exists in the inventory, that every `linked_rule_ids` entry exists in the QuanShu rule corpus, that linked items and rules agree on `source_id` and `work`, and that a source item's text **equals** the linked rule's `source_text_zh_hans` after light punctuation normalization (a `notes_zh_hans` does not bypass this). It also locks the 天马空亡 source unit to `马遇空亡，终身奔走`, the 禄马交驰 unit to `禄马最喜交驰`, and the 日月反背 unit to `日月最嫌反背`. Pattern catalogue entries remain `segmented` and unlinked in source inventory. Implemented canonical `PatternId` provenance metadata must reference valid QuanShu source inventory items with matching source text. Project-owned pattern rules in `crates/iztro/rule-corpus/patterns/rules.toml` keep `work = "iztro_pattern_catalog"` and `pattern.*` source ids. There are no QuanShu-sourced pattern runtime rules.
