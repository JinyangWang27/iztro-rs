//! JSON key parsing and normalized-field assertion helpers shared by the
//! fixture-backed tests. These translate normalized snake_case fixture keys
//! into typed values and assert a built `Chart` against a `supported_fields`
//! block.
...
use iztro::core::{
    ...
    FlowStarBase, Gender, HeavenlyStem, Mutagen, PalaceName, StarKind, StarName,
    known_star_metadata_table, represented_star_metadata_table,
};
...
/// Alias for [`parse_gender`]; some fixtures historically used this name.
pub fn parse_gender_key(value: &str) -> Gender {
    parse_gender(value)
}
