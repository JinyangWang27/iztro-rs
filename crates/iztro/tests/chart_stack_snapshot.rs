...
fn natal_chart_stack_snapshot_preserves_renderer_ready_natal_facts() {
    ...
    assert_eq!(snapshot.four_pillars(), chart.four_pillars());
    assert_eq!(
...
fn horoscope_chart_stack_snapshot_groups_temporal_overlays_by_branch_without_duplicating_natal_stars()
 {
    ...
    assert_eq!(snapshot.four_pillars(), natal.four_pillars());
    ...
    assert_eq!(
        horoscope.natal().four_pillars(),
        natal.four_pillars(),
        "temporal overlays should not mutate natal four pillars"
    );
}
