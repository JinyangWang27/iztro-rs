...
//!
//! 成格: both 太阳 and 太阴 are present and each sits in a clearly dim/fallen state
//! (不/陷). If either brightness is `Unknown` (or merely `Flat`/bright), nothing is
//! emitted — the rule never guesses an uncalculated brightness.
...
use crate::rules::pattern::predicates::brightness::is_dim;
...
/// 成格: 太阳 and 太阴 both present and both dim/fallen.
